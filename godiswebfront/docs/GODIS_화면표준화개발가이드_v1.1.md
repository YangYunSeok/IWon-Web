All Admin Web screen components must be located under the `screens/` directory.
The `pages/` directory is not used for screen implementation.


# GODIS 화면 표준화 개발가이드 (v 1.1)
최초작성일: 2025-12-22
최종수정일: 2026-01-09
...
본 장에서는 GODIS 관리자 웹에서 사용하는 **차트 화면의 표준 구성, 공통 컴포넌트, 차트 타입별 속성 규칙**을 정의한다.

차트는 관리자에게 **요약·추세·비율 정보를 직관적으로 전달**하기 위한 시각적 요소이며,
**정확한 수치 확인은 반드시 그리드(GDataGrid)로 병행 제공**해야 한다.
...
* ✅ 차트는 **요약/분석용**, 그리드는 **정확한 데이터 확인용**
* ✅ 차트 단독 화면 구성 ❌ (반드시 그리드와 함께 사용)
* ✅ 차트는 화면 상단 요약 영역에 배치
* ❌ 동일 데이터를 여러 차트로 중복 표현 금지
* ❌ 과도한 색상/시리즈 사용 금지
...
차트 영역은 `GContentBox`를 사용하여 표준화한다.
...
## 7.3 차트 공통 래퍼 컴포넌트 – GChartBox

### 7.3.1 GChartBox 사용 목적

GChartBox는 차트 UI의 **레이아웃·타이틀·상태·액션 버튼**을 공통화하기 위한 래퍼 컴포넌트이다.
...
<GChartBox
  title="일자별 코인 사용량"
  subtitle="기간: 2026-01-01 ~ 2026-01-31"
  height={260}
  loading={loading}
  empty={!loading && data.length === 0}
  error={error}
  actions={[
    <GButton key="refresh" auth="Search" label="Refresh" onClick={reload} />
  ]}
>
  <GLineChart
    data={data}
    xKey="date"
    yKeys={[{ key: 'amount', name: '사용량', unit: 'IWC' }]}
  />
</GChartBox>
```
...
### 7.3.3 GChartBox Props 표준

| 파라미터       | 타입                 | 기본값   | 설명           |
| ---------- | ------------------ | ----- | ------------ |
| `title`    | string             | 필수    | 차트 제목        |
| `subtitle` | string | ReactNode | null  | 기간 / 단위 / 설명 |
| `height`   | number             | 260   | 차트 영역 높이(px) |
| `loading`  | boolean            | false | 로딩 상태        |
| `empty`    | boolean            | false | 데이터 없음 상태    |
| `error`    | string | ReactNode | null  | 오류 메시지       |
| `actions`  | ReactNode[]        | []    | 우측 상단 액션 버튼  |
| `padding`  | number             | 8     | 내부 패딩        |
| `sx`       | object             | `{}`  | 스타일 확장       |
...
```js
[
  { date: '2026-01-01', amount: 1200, approved: 900, rejected: 300 },
  { date: '2026-01-02', amount: 800,  approved: 700, rejected: 100 }
]
```

* `xKey`: X축 기준 필드 (날짜 / 카테고리)
* `yKeys`: Y축 값(1개 이상)

---

## 7.5 선 그래프 – GLineChart
...
<GLineChart
  ...
  xKey="date"
  yKeys={[
    { key: 'amount', name: '사용량', unit: 'IWC' },
    { key: 'approved', name: '승인', unit: '건' }
  ]}
  showGrid
  showTooltip
  showLegend
...
| 파라미터          | 타입      | 기본값  | 설명        |
| ------------- | ------- | ---- | --------- |
| `data`        | array   | 필수   | 차트 데이터    |
| `xKey`        | string  | 필수   | X축 키      |
| `yKeys`       | array   | 필수   | 시리즈 정의    |
| `showGrid`    | boolean | true | 배경 그리드    |
| `showTooltip` | boolean | true | 툴팁 표시     |
| `showLegend`  | boolean | true | 범례 표시     |
| `height`      | number  | 220  | 차트 높이(px) |

---

## 7.6 영역 그래프 – GAreaChart / GStackedAreaChart

### 기본 영역 그래프 (GAreaChart)
...
<GDonutChart
  ...
  showLegend
  showCenterLabel
  centerLabelFormatter={(total) => `총 ${total}건`}
/>
```
...
* ❌ 파이/도넛 차트에 항목 6개 이상 금지
* ❌ 누적 차트에서 시리즈 5개 초과 금지
* ❌ 차트 높이 300px 초과 금지
* ❌ 차트만 있는 단독 화면 금지

---

## 7.10 Copilot 차트 생성 규칙

* 차트는 반드시 `GChartBox`로 감싼다.
* 표준 차트 컴포넌트만 사용한다.
* 차트가 있으면 **그리드를 함께 제안**한다.
* 차트 Props는 본 장의 표준을 따른다.
