...
정책(결정): 거래 이력 조회는 **관리자 전용 API(`/admin/transactions`)로 신설**합니다.

- 이유: 권한(SoD)·감사 목적 조회를 일반 사용자 거래 API와 분리해 데이터 노출/필터/감사 로직을 명확히 하기 위함
- 비고: 내부 구현에서 필요하면 기존 원장 테이블/쿼리(`wallet_transactions`)를 재사용하되, **외부 계약(엔드포인트)은 분리 유지**
