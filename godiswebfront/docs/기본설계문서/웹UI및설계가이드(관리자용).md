...
이 문서는 **IWon 스테이블 코인 관리 시스템(Admin Web)**의 현행 UI/UX 가이드입니다. (업데이트 기준일: 2026-01-08)
...
* **관리자 콘솔 (LNB/Sidebar 구조)**
    ...
    * **온체인 승인 관리 (`Approval`)**: 기안/승인 워크플로우
    * **월별 지급 관리 (`MonthlyPlan`)**: 정기 지급 계획 및 이관
...
## 7. 온체인 승인 관리 (`Approval`)

코인 발행·회수에 대한 **사전 통제와 최종 실행**을 담당하는 승인 화면입니다.
