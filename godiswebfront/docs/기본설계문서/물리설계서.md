...
UI 요구사항은 “탭 1 분개 리스트 / 탭 2 결산 요약 / 탭 3 준비금·발행부채(코인 분리)”이며,  
...
#### 5.1.2 financial_journal_entries (탭 1: 분개 리스트)
...
CREATE TABLE IF NOT EXISTS financial_journal_entries (
  ...
  event_type      varchar(30) NOT NULL,     -- mint/burn/transfer/conversion/settlement 등
  coin_type       varchar(50) NOT NULL REFERENCES coin_types(coin_type),
...
#### 5.1.3 financial_reports (탭 2: 결산보고서 요약)
...
#### 5.1.4 reserve_liability_snapshots (탭 3: 준비금/발행부채)
