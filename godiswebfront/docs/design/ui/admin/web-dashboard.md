...
- 로그인 직후 또는 Sidebar `Dashboard` 클릭.
- Focus/새로고침 시 `GET /admin/stats/supply`, `GET /admin/stats/daily` 동시 호출.
...
- `체크 시각 새로고침` 버튼 → 두 통계 API 재호출.
- Alert 항목 클릭 → 관련 화면 (Approval / WalletMgmt) 딥링크.
