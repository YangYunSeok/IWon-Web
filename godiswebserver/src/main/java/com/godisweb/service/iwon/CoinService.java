...
public class CoinService {
    ...
    public Map<String, Object> getAdminWalletInfo() {
        ...
        try {
            ...
            // ERC20 token = ERC20.load(CONTRACT_ADDR, web3j, credentials, new
            // DefaultGasProvider());

            // Wei 단위(10^18)를 보기 좋게 변환
            // BigInteger balanceWei = token.balanceOf(ADMIN_ADDR).send();
            // BigDecimal balanceEth = new
            // BigDecimal(balanceWei).divide(BigDecimal.TEN.pow(18));
    ...
    public Map<String, Object> syncBalances() {
        ...
        try {
            ...
            // 병렬 스트림 실행
            customThreadPool.submit(() -> list.parallelStream().forEach(emp -> {
    ...
    public Map<String, Object> transferCoin(Map<String, Object> param) {
        ...
        new Thread(() -> {
            try {
                ...
                // BigDecimal newBalanceEth = new
                // BigDecimal(newBalanceWei).divide(BigDecimal.TEN.pow(18));
